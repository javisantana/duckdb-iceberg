...
public:
	explicit IcebergInsertGlobalState(ClientContext &context, IcebergTableEntry *table) : insert_count(0) {
	}
...
unique_ptr<PhysicalOperator> IcebergCatalog::PlanInsert(ClientContext &context, LogicalInsert &op,
                                                       unique_ptr<PhysicalOperator> plan) {
	printf("ICEBERG PLAN INSERT\n");
	if (op.return_chunk) {
	...
	//auto insert = make_uniq<IcebergInsert>(op, op.table, op.column_index_map);
	//insert->children.push_back(std::move(plan));
	//return std::move(insert);
    return plan;
}

unique_ptr<PhysicalOperator> IcebergCatalog::PlanCreateTableAs(ClientContext &context, LogicalCreateTable &op,
                                                              unique_ptr<PhysicalOperator> plan) {
	//plan = AddCastToSQLiteTypes(context, std::move(plan));
